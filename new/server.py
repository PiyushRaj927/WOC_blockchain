...
class GossipNode:
    ...
    def __init__(self, port, connected_nodes,name):
        ...
        self.nodes_list = connected_nodes
        self.hostname = 'localhost'
        self.susceptible_nodes = self.nodes_list
        self.infected_nodes = []
        self.port = port
        self.name = name
        ...
        self.start_threads()
    def update_nodes_list(self,list):
...
"""
    ...
    def transmit_message(self, message):
        ...
        for selected_port in self.nodes_list:
            
                message_sig = self.identity.signTransaction(bytes(json.dumps(message),'utf-8')).decode('utf-8')
                print(message_sig)
                message_to_forward = json.dumps({'update_id':self.create_id(message),
                                                    'data':message,
                                                    'sig':message_sig,
                                                    'id':self.identity.PublicKey.decode('utf-8'),
                                                    'Timestamp':time.time()
                                                    }).encode("utf-8")
    ...
    def has_update(self,port):
        pass
    def send_update(self,port):
      pass  
