...
class Blockchain: 
    def __init__(self,database_list):
        ...
        Thread(target=self.push_block_mongo).start()
    def len(self):
    ...
    def create_genesis_block(self):
        ...
        self.current_index = self.databaselist[0].insert_one_block(genesis_block)
    def last_block(self):
        return self.chain[-1]
    ...
    def add_block(self, block):
        ...
        mongo.ledger_db.insert_one_block(block)
            ...
            for k in node.Gossip.cmd:
                if k['type']=='block_verification':
                    ...
                    if k['data'] == 'True':
                        ...
                        node.Gossip.cmd.remove(k)"""
    def push_block_mongo(self):
        if len(self.chain) > 2:
            
            mongo.ledger_db.insert_one_block(self.chain[0])
            del self.chain[0]
    def sync_to_databse():
        pass
